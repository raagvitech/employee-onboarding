...
import insertTaskComment from '@salesforce/apex/employeeViewForTrainingHandler.createTaskComment';
...
export default class TaskComponent extends NavigationMixin(LightningElement)  {
    ...
    customShowModalPopupforQueries(){
        this.openModalToAddQueries = true;
    }
    ...
    customHideModalPopup() {     
        ...
        this.openModalToAddQueries = false;
    }
    connectedCallback(){
        this.getTaskData();
    }
    ...
    if(event.target.label=='Task Number')
    {
        this.createTaskComment.Name	 = event.target.value;
    }
    if(event.target.label=='Comment')
    {
        this.createTaskComment.Comment__c	 = event.target.value;
    }
    ...
    {
      this.newlist.Priority = event.target.value;
      window.console.log('Description ==> '+this.newlist.Description);
    }
    ...
    createTaskCommentHandler(event){
        console.log('createTaskCommentHandler:::::',JSON.stringify(this.createTaskComment));
        var valid = this.inputValidation();
        if(!valid){
            return;
        }
        insertTaskComment({taskComment : this.createTaskComment})
        .then(result=>{
    ...
    handelOnClick()
    {
       console.log('i am in handle on click');
       console.log('description----------'+this.newlist.Description, " => ",this.inputValidation());
        var valid = this.inputValidation();
        if(!valid){
            return;
        }

       if(this.newlist.Description && this.newlist.Status && this.newlist.Priority){
         console.log('log i am in inside of if');
    ...
    inputValidation(){
        ...
        let isValid = true;
        var arrayOfFields = this.template.querySelectorAll('.validation');
